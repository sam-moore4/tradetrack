    ...
    <Box m="1.5rem 2.5rem">
      <Box>
        <Header
          m="1.5rem"
          title="ACTIVE"
          subtitle="Here are your active trades"
        />
      </Box>
      <Box m="1.5rem 2.5rem">
        <TableContainer
          ...
          <Table>
            ...
            <TableBody>
              {filteredTrades.map((trade) => {
                ...
                const getCurrentValue = (lastsale, quantity) => {
                  return isNaN(lastsale) || isNaN(quantity)
                    ? "-"
                    : formatCurrency(lastsale * quantity);
                };
                ...
                return (
                  <TableRow key={trade._id}>
                    ...
                    <TableCell>
                      {getCurrentValue(
                        parseFloat(trade.lastsale.slice(1)),
                        trade.quantity
                      )}
                    </TableCell>
            ...
            <TableRow>
                ...
                <strong>
                  {formatCurrency(
                    filteredTrades.reduce(
                      (total, trade) =>
                        total +
                        parseFloat(trade.lastsale.slice(1)) * trade.quantity,
                      0
                    )
                  )}
                </strong>
              ...
              <TableCell>
                <strong>
                  {formatCurrency(
                    filteredTrades.reduce((total, trade) => {
                      const { price, lastsale, direction, quantity } = trade;
                      const pl =
                        formatDirection(direction) === "Sell"
                          ? (parseFloat(price) -
                              parseFloat(lastsale.slice(1))) *
                            quantity
                          : (parseFloat(lastsale.slice(1)) -
                              parseFloat(price)) *
                            quantity;
